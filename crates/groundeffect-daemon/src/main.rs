...
struct Cli {
    ...
    /// Enable file logging to ~/.local/share/groundeffect/logs/daemon.log
...
#[derive(Subcommand)]
enum Commands {
    /// Add a new Google account via OAuth (DEPRECATED: use 'groundeffect account add')
    AddAccount {
        /// Optional alias for the account (e.g., "work", "personal")
        #[arg(short, long)]
        alias: Option<String>,
    },
    /// List all configured accounts (DEPRECATED: use 'groundeffect account list')
    ListAccounts,
    /// Remove an account (DEPRECATED: use 'groundeffect account delete')
    RemoveAccount {
...
async fn add_account(alias: Option<String>) -> Result<()> {
    ...
    eprintln!("Please use: groundeffect account add\n");
...
<body style="font-family: -apple-system, BlinkMacSystemFont, sans-serif; padding: 40px; text-align: center;">
    ...
    if let Some(existing) = db.get_account(&user_info.email).await? {
        println!("Account {} already exists, updating tokens...", existing.id);
    ...
    if let Some(ref a) = db
        .get_account(&user_info.email)
        .await?
        .and_then(|acc| acc.alias)
    {
        println!("   Alias: {}", a);
    }
    println!("\nYou can now run the daemon to start syncing:");
    println!("  groundeffect-daemon\n");
...
async fn list_accounts() -> Result<()> {
    eprintln!("\nDEPRECATED: This command is deprecated.");
    eprintln!("Please use: groundeffect account list\n");
    ...
    if !config.lancedb_dir().exists() {
        ...
        println!("\nAdd an account with:");
        println!("  groundeffect account add\n");
        return Ok(());
    }
...
async fn remove_account(account: &str) -> Result<()> {
    eprintln!("\nDEPRECATED: This command is deprecated.");
    eprintln!("Please use: groundeffect account delete <account>\n");
    ...
    let target = accounts
        .iter()
        .find(|a| a.id == account || a.alias.as_ref().map(|al| al == account).unwrap_or(false));
...
async fn run_daemon() -> Result<()> {
    info!(
        "Starting GroundEffect daemon v{}",
